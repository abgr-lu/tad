...
export default function DashboardLayout({ children }) {
  ...
  return (
    <div className="flex min-h-screen bg-[#F8FAFC] dark:bg-[#0B1120] font-sans text-slate-900 dark:text-slate-100 antialiased transition-colors duration-200 selection:bg-blue-500/30">
      ...
      <nav className="w-64 bg-white dark:bg-[#0F172A] border-r border-slate-200 dark:border-slate-800/60 p-6 flex flex-col justify-between fixed h-screen shrink-0 z-10 transition-colors duration-200">
        <div className="flex flex-col flex-1">
          <div className="border-b border-slate-200 dark:border-slate-800/60 pb-6 mb-6">
            
            {/* LOGOTIPO ACTUALIZADO CON ESTILO DE LA LANDING PAGE */}
      ...
      <main className="flex-1 ml-64 transition-colors duration-200 overflow-y-auto min-h-screen relative bg-[#F8FAFC] dark:bg-[#0B1120]">
        <div className={`p-10 max-w-7xl mx-auto transition-all duration-500 ${isExpired ? 'blur-[8px] pointer-events-none select-none opacity-50' : ''}`}>
          {children}
        </div>

        {isExpired && (
          <div className="absolute inset-0 z-50 flex items-start justify-center pt-[15vh]">
            <div className="bg-white dark:bg-[#0F172A] border border-slate-200 dark:border-slate-800 p-10 rounded-2xl shadow-2xl max-w-lg w-full text-center mx-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <button 
                  ...
                  disabled={loadingPriceId === process.env.NEXT_PUBLIC_STRIPE_PRICE_ID_MONTHLY}
                ...
                <button 
                  ...
                  disabled={loadingPriceId === process.env.NEXT_PUBLIC_STRIPE_PRICE_ID_ANNUAL}
